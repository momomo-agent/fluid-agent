...
For conversation, questions, opinions, brainstorming — just reply normally. No action blocks needed. Be natural, concise, and have personality.`
  ...
  async function startWorker(taskDescription, plannedSteps, abort) {
    ...
    const toolDefs = {
      ...
      app: { desc: 'Manage generative apps: create with HTML/CSS/JS, update, uninstall, list. Apps run in sandboxed windows and get installed in the dock. ALWAYS set width/height appropriate for the content: calculator~320x420, text tool~500x400, dashboard~700x500, game~600x500.', schema: { type: 'object', properties: { action: { type: 'string', enum: ['create', 'update', 'uninstall', 'list'] }, name: { type: 'string' }, html: { type: 'string' }, css: { type: 'string' }, js: { type: 'string' }, icon: { type: 'string' }, width: { type: 'number' }, height: { type: 'number' }, description: { type: 'string' } }, required: ['action'] } },
